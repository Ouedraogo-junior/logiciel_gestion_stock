...
export default function DashboardClient({ initialData }: { initialData: DashboardData }) {
  ...
  const maxQty = Math.max(...initialData.top5.map(p => p.qty), 1)
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-5">
          ...
          <p className="text-3xl font-bold text-red-700 mt-1">
            {initialData.totalDettes.toLocaleString()}
          </p>
          <p className="text-xs text-red-400 mt-1">FCFA à recouvrer</p>
          {initialData.totalDettes > 0 && (
            <button
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {initialData.lowStock.length > 0 && (
            <span className="bg-red-100 text-red-700 text-xs font-medium px-2 py-0.5 rounded-full">
              {initialData.lowStock.length} article{initialData.lowStock.length > 1 ? 's' : ''}
            </span>
          )}
        </div>
        {initialData.lowStock.length === 0 ? (
